...
def get_training_transform():
    # Match paper settings: random vertical flip, random horizontal flip, and random rotation
    train_transform = [
        albu.VerticalFlip(p=0.5),     # 原文：random vertical flip
        albu.HorizontalFlip(p=0.5),   # 原文：random horizontal flip  
        albu.RandomRotate90(p=0.5),   # 原文：random rotation
        albu.Normalize()
    ]
...
def val_aug(img, mask):
    # 验证时也使用1024裁剪，确保尺寸一致
    crop_aug = Compose([SmartCropV1(crop_size=1024, max_ratio=0.75,
                                    ignore_index=len(CLASSES), nopad=False)])
    img, mask = crop_aug(img, mask)
    img, mask = np.array(img), np.array(mask)
...
class VaihingenDataset(Dataset):
    def __init__(self, data_root='/root/autodl-tmp/Vaihingen/test', mode='val', img_dir='images', mask_dir='masks',
